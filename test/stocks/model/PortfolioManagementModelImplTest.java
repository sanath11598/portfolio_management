...
public class PortfolioManagementModelImplTest {
  ...
  @Test
  public void testFlexiblePortfolioBuyByDCAValueMultipleDates() throws Exception {
    Portfolio portfolio = new FlexiblePortfolioImpl("portfolioName", strDate);
    PortfolioManagementModel pm = new PortfolioManagementModelImpl(username, portfolio);
    String tickerList = "goog,aapl";
    String weightList = "50,50";
    ...
    PortfolioOperation<List<TransactionItem>> dca = new BuyBySip<>("portfolioName", username,
            dates, tickerList, weightList, 1000, 0, transactions);
    transactions = pm.accept(dca);
    List<List<String>> valuation = pm.constructPortfolioValuation("2021-01-01");
    List<List<String>> expected1 = Arrays.asList(
            new ArrayList<>(Arrays.asList("10443.2")),
            new ArrayList<>(Arrays.asList("goog", "68.12", "6501.37")),
            new ArrayList<>(Arrays.asList("aapl", "30.46", "3941.83"))
    );
    assertTrue(expected1.size() == valuation.size()
            && expected1.containsAll(valuation) && valuation.containsAll(expected1));
    valuation = pm.constructPortfolioValuation("2022-01-01");
    ...
    List<List<String>> expected2 = Arrays.asList(
            new ArrayList<>(Arrays.asList("25783.91")),
            new ArrayList<>(Arrays.asList("goog", "131.0", "12502.64")),
            new ArrayList<>(Arrays.asList("aapl", "72.97", "13281.27"))
    );
    assertTrue(expected2.size() == valuation.size()
            && expected2.containsAll(valuation) && valuation.containsAll(expected2));
  }
}